...
const Header = () => {
  ...
  const notice_popup = useSelector(
    (state: RootState) => state.app.notice_popup,
  );
  ...
  const handleCookieRenew = async () => {
    try {
      const response = await axios.get(`${apiUrl}/login/jwt/${userId}`, {
  ...
  const settings = isAdmin
    ? [{ name: translations.account, path: `/account` }, `JWT`, `Logout`]
    : [
  ...
  return (
    <AppBar position="static">
      ...
      {notice_popup.on && (
        <NoticePopUpOn dispatch={dispatch} notice_popup={notice_popup} />
      )}
    </AppBar>
  );
};
