...
const AdminDataManage = () => {
  ...
  const [filteredInfos, setFilteredInfos] =
    useState<CountryVaccineDataResponse | null>(null);
  ...
  const mainDataFetch = async () => {
    try {
      const response = await axios.get<CountryVaccineDataResponse>(
        `${apiUrl}/admin/manage`,
        {
    ...
    <Container className="Main center_ui">
      <Container className={'main_top'}>
        <div className={'mt_elem'}>
          <p className={'main_greeting'}>{translations.admin_manage}</p>
        </div>
      </Container>
      {filteredInfos && (
        <CountryVaccineInformation
          dispatch={dispatch}
          apiUrl={apiUrl}
