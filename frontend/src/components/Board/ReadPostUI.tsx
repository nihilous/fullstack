...
const ReadPostUI = (props: ReadPostUIProps) => {
  ...
  const writeReply = async (post_id: number) => {
    try {
      const response = await axios.post(
        `${apiUrl}/board/${post_id}`,
        { user_id: userId, text: newReply },
        {
  ...
  const deletePostReply = async (
    ...
    try {
      const response = is_admin
        ? await axios.delete(`${apiUrl}/admin/${table}/${post_id}`, {
            headers: { Authorization: `Bearer ${getToken()}` },
          })
  ...
  const modifyPostOrReply = (
    table: string,
    edit_target_id: number,
    edit_target_title: string | null,
    edit_target_text: string,
  ) => {
    if (table === 'post') {
      setEditReply(false);
  ...
  const updatePostOrReply = async (edit_target_table: string) => {
    ...
    try {
      const response = await axios.put(
        `${apiUrl}/board/${edit_target_table}/${edit_target_table === 'post' ? editPostNum : editReplyNum}`,
        edit_target_table === 'post'
          ? { user_id: userId, title: editPostTitle, text: editPostText }
          : { user_id: userId, text: editReplyText },
        {
          headers: { Authorization: `Bearer ${getToken()}` },
        },
      );
      ...
      if (axiosError.response) {
        ...
        switch (BoardUpdateRes) {
          ...
          case 4:
            message =
              edit_target_table === 'post'
                ? popupTranslations.BoardWriteRequire
                : popupTranslations.BoardUpdateNotExist;
            break;
          default:
            const checkRes = jwtChecker(
              error as AxiosError<{ tokenExpired: boolean }>,
      ...
      reply_elem.push(
        <div key={parsed_repies[i].reply_id} className={'reply_elem'}>
          <div className={'rep_info'}>
            <div className={'rep_writer'}>
              {parsed_repies[i].reply_is_admin
                ? translations.admin
                : parsed_repies[i].reply_user_nickname === null
                  ? translations.quit
                  : parsed_repies[i].reply_user_nickname}
            </div>
            ...
            <div className={'rep_date_delete'}>
              <div>
                {formatDate(parsed_repies[i].reply_updated_at, language)}
              </div>
            </div>
          </div>
          {parsed_repies[i].reply_user_id === userId &&
          Boolean(parsed_repies[i].is_admin) === admin ? (
            <div className={'rep_interaction'}>
              <div>
                {editReply && editReplyNum === parsed_repies[i].reply_id ? (
                  <Button onClick={() => updatePostOrReply('reply')}>
                    {translations.save}
                  </Button>
                ) : (
                  <Button
                    onClick={() =>
                      modifyPostOrReply(
                        'reply',
                        parsed_repies[i].reply_id,
                        null,
                        parsed_repies[i].reply_text,
                      )
                    }
                  >
                    {translations.edit}
              ...
              <div>
                <Button
                  onClick={() =>
                    deletePostReply('reply', parsed_repies[i].reply_id, false)
                  }
                >
                  {translations.delete}
                </Button>
            ...
            <div className={'rep_interaction'}>
              <div>
                <Button
                  onClick={() =>
                    deletePostReply('reply', parsed_repies[i].reply_id, admin)
                  }
                >
                  {translations.delete}
                </Button>
  ...
  return (
    <div key={`post_view_${post_info.id}`} className={'post_wrap'}>
      <div className={'post_main'}>
        ...
        Boolean(post_info.is_admin) === admin ? (
          <div className={'pm_bot'}>
            <div>
              {editPost ? (
                <Button onClick={() => updatePostOrReply('post')}>
                  {translations.save}
                </Button>
              ) : (
                <Button
            ...
            <div>
              <Button
                onClick={() => deletePostReply('post', post_info.id, false)}
              >
                {translations.delete}
              </Button>
            </div>
          ...
          <div className={'pm_bot'}>
            <div>
              <Button
                onClick={() => deletePostReply('post', post_info.id, admin)}
              >
                {translations.delete}
              </Button>
            </div>
