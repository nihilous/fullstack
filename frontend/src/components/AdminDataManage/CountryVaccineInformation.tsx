...
interface CountryVaccineInformationProps {
  dispatch: Dispatch<any>;
  apiUrl: string;
  translations: any;
  popupTranslations: any;
  mainDataFetch: Function;
  deleteVaccineRecord: Function;
  deleteCountry: Function;
  info: CountryVaccineDataResponse;
}

const CountryVaccineInformation = (props: CountryVaccineInformationProps) => {
  const dispatch = props.dispatch;
  const apiUrl = props.apiUrl;
  const translations = props.translations;
  const popupTranslations = props.popupTranslations;
  const mainDataFetch = props.mainDataFetch;
  const deleteVaccineRecord = props.deleteVaccineRecord;
  const deleteCountry = props.deleteCountry;
  ...
  const [editVaccineNationalCode, setEditVaccineNationalCode] = useState<
    number | null
  >(null);
  const [editVaccinePeriodical, setEditVaccinePeriodical] = useState<number>(2);
  const [editVaccineMinPeriodType, setEditVaccineMinPeriodType] =
    useState<string>(``);
  const [editVaccineMinPeriod, setEditVaccineMinPeriod] = useState<
    number | null
  >(null);
  const [editVaccineMaxPeriodType, setEditVaccineMaxPeriodType] =
    useState<string>(``);
  const [editVaccineMaxPeriod, setEditVaccineMaxPeriod] = useState<
    number | null
  >(null);
  const [editVaccineRound, setEditVaccineRound] = useState<number | null>(null);
  const [editVaccineDescription, setEditVaccineDescription] =
    useState<string>(``);

  const [vaccineName, setVaccineName] = useState<string | null>(``);
  const [vaccineNationalCode, setVaccineNationalCode] = useState<number | null>(
    null,
  );
  ...
  const saveModifiedCountry = async (id: number) => {
    try {
      const params = {
        ori_id: id,
        new_id: editNationalId,
        code: editNationalCode,
        eng: editNationalEng,
        ori: editNationalOri,
      };
      await axios.put<CountryVaccineDataResponse>(
        `${apiUrl}/admin/manage/update/country`,
        params,
        {
  ...
  const saveAddCountry = async () => {
    try {
      const params = {
        id: addNationalId,
        code: addNationalCode,
        eng: addNationalEng,
        ori: addNationalOri,
      };
      await axios.post<CountryVaccineDataResponse>(
        `${apiUrl}/admin/manage/add/country`,
        params,
        {
          headers: { Authorization: `Bearer ${getToken()}` },
        },
  ...
  const addVaccineData = async () => {
    try {
      ...
      await axios.post<CountryVaccineDataResponse>(
        `${apiUrl}/admin/manage/add/vaccine`,
        params,
        {
          headers: { Authorization: `Bearer ${getToken()}` },
        },
  ...
  const saveModifiedVaccine = async () => {
    try {
      ...
      await axios.put<CountryVaccineDataResponse>(
        `${apiUrl}/admin/manage/update/vaccine`,
        params,
        {
          headers: { Authorization: `Bearer ${getToken()}` },
        },
  ...
  return (
    <>
      <div>
        <div className={`adminMangeTitle`}>{translations.existing_country}</div>
        <div className={`adminManageCountryHeader`}>
          <div>{translations.id}</div>
          <div>{translations.national_code}</div>
          <div>{translations.name_english}</div>
          <div>{translations.name_original}</div>
        </div>
        ...
        <div className={`adminManageCountry`}>
          {info.existing_countries.map(country => (
            <div key={country.id} className={`amc_wrap`}>
              <div>{country.id}</div>
              <div>{country.national_code}</div>
              <div>{country.name_english}</div>
              <div>{country.name_original}</div>
            </div>
          ))}
        </div>

        <div className={`adminMangeTitle`}>{translations.temporal_country}</div>
              ...
              <div key={country.id} className={`amc_edit_wrap`}>
                <div>
                  <Form.Control
                    ...
                    onChange={e =>
                      e.target.value !== ''
                        ? setEditNationalId(parseInt(e.target.value, 10))
                        : setEditNationalId(null)
                    }
                  />
                </div>
                <div>
                ...
                <div>
                  <Button onClick={() => saveModifiedCountry(country.id)}>
                    {translations.save}
                  </Button>
                </div>
                <div>
                  <Button onClick={() => cancelModifiedCountry(country.id)}>
                    {translations.cancel}
                  </Button>
                </div>
              </div>
        ...
        <div className={`adminMangeTitle`}>{translations.temporal_vaccine}</div>

        <div style={{ marginTop: '40px' }}>
          <div
            className={`manageTempVacHeader`}
              ...
              <div key={vaccine.id} className={`manageTempVacElem`}>
                  ...
                  <Tooltip title={`temporal vaccine name`}>
                    ...
                    >
                      ...
                      {info.existing_vaccines.map(vaccine => (
                        <option key={vaccine.id} value={vaccine.vaccine_name}>
                ...
                <div>
                  <Tooltip title={`temporal vaccine is periodical?`}>
                    <Form.Select
                      onChange={e =>
                        setEditVaccinePeriodical(parseInt(e.target.value))
                      }
                ...
                <div>
                  <Tooltip
                    title={`temporal vaccine minimum period type (month or year)`}
                  >
                    <Form.Select
                      onChange={e =>
                        setEditVaccineMinPeriodType(e.target.value)
                      }
                      value={
                        editVaccineMinPeriodType !== ''
                          ? (editVaccineMinPeriodType as string)
                          : ''
                      }
                      size="sm"
                      className="footer_language"
                ...
                <div>
                  <Tooltip title={`temporal vaccine minimum recommended date`}>
                    <Form.Select
                      onChange={e =>
                        setEditVaccineMinPeriod(parseInt(e.target.value, 10))
                      }
                      value={
                        editVaccineMinPeriod !== null
                          ? editVaccineMinPeriod.toString()
                          : ''
                      }
                      size="sm"
                      className="footer_language"
                    >
                      ...
                      {editVaccineMinPeriodType && (
                        <PeriodOptions
                          isPeriod={true}
                          periodType={editVaccineMinPeriodType}
                        />
                      )}
                    </Form.Select>
                  </Tooltip>
                ...
                <div>
                  <Tooltip
                    title={`temporal vaccine maximum period type (month or year)`}
                  >
                    <Form.Select
                      onChange={e =>
                        editVaccinePeriodical === 1
                          ? setEditVaccineMaxPeriodType(e.target.value)
                          : setEditVaccineMaxPeriodType('')
                      }
                      value={
                        editVaccineMaxPeriodType !== ''
                          ? (editVaccineMaxPeriodType as string)
                          : ''
                      }
                      size="sm"
                      className="footer_language"
                ...
                <div>
                  <Tooltip title={`temporal vaccine maximum recommended date`}>
                    <Form.Select
                      onChange={e =>
                        editVaccinePeriodical === 1
                          ? setEditVaccineMaxPeriod(
                              parseInt(e.target.value, 10),
                            )
                          : setEditVaccineMaxPeriod(null)
                      }
                      value={
                        editVaccineMaxPeriod !== null
                          ? editVaccineMaxPeriod.toString()
                          : ''
                      }
                      size="sm"
                      className="footer_language"
                      ...
                      {editVaccinePeriodical !== 1 ? (
                        ...
                        <>
                          ...
                          {editVaccineMaxPeriodType && (
                            <PeriodOptions
                              isPeriod={true}
                              periodType={editVaccineMaxPeriodType}
                            />
                          )}
                        </>
                      )}
                ...
                <div>
                  <Tooltip title={`temporal vaccine round of dose`}>
                    <Form.Select
                      onChange={e =>
                        setEditVaccineRound(parseInt(e.target.value, 10))
                      }
                      value={
                        editVaccineRound !== null
                          ? editVaccineRound.toString()
                          : ''
                      }
                      size="sm"
                      className="footer_language"
                    >
                      ...
                      {<PeriodOptions isPeriod={false} periodType={null} />}
                    </Form.Select>
                  </Tooltip>
                </div>
                <div>
                ...
                <div>
                  <Button onClick={() => saveModifiedVaccine()}>
                    {translations.save}
                  </Button>
                </div>
                <div>
                  <Button onClick={() => cancelModifiedVaccine()}>
                    {translations.cancel}
                  </Button>
                </div>
              </div>
              ...
              >
                ...
                <div>
                  <Tooltip title={`Minimum Recommended Date`}>
                    <span>
                      ...
                      {vaccine.vaccine_minimum_period_type === 'M' ? 'M' : 'Y'}
                    </span>
                  </Tooltip>
                </div>
                <div>
                  ...
                  >
                    <span>
                      {vaccine.vaccine_maximum_period_type === null
                        ? ''
                ...
                <div>
                  <Tooltip title={`Maximum Recommended Date`}>
                    <span>
                      {vaccine.vaccine_maximum_recommend_date === null
                        ? ''
                  ...
                  <Button
                    onClick={() =>
                      editVaccineInfo(
                        ...
                        vaccine.vaccine_minimum_recommend_date,
                        vaccine.vaccine_maximum_period_type !== null
                          ? (vaccine.vaccine_maximum_period_type as string)
                          : '',
                        vaccine.vaccine_maximum_recommend_date !== null
                          ? (vaccine.vaccine_maximum_recommend_date as number)
                          : null,
                ...
                <div>
                  <Button onClick={() => deleteVaccineRecord(vaccine.id)}>
                    {translations.delete}
                  </Button>
                </div>
              </div>
              ...
              <>
                <div>
                  <Form.Control
                    ...
                    onChange={e =>
                      e.target.value !== ''
                        ? setAddNationalId(parseInt(e.target.value, 10))
                        : setAddNationalId(null)
                    }
                  />
                </div>
                <div>
                  <Form.Control
                    ...
                    onChange={e =>
                      setAddNationalCode(e.target.value.toUpperCase())
                    }
                  />
                </div>
                <div>
                ...
                <div>
                  <Button onClick={() => saveAddCountry()}>
                    {translations.save}
                  </Button>
                </div>
                <div>
                  <Button onClick={() => cancelAddCountry()}>
                    {translations.cancel}
                  </Button>
                </div>
              </>
            ...
            <Form.Select
              ...
              value={
                vaccineMinPeriodType !== ''
                  ? (vaccineMinPeriodType as string)
                  : ''
              }
              size="sm"
              className="footer_language"
        ...
        <div>
          <Tooltip title={translations.vaccine_minimum_recommend_date}>
            <Form.Select
              ...
              value={
                vaccineMinPeriod !== null ? vaccineMinPeriod.toString() : ''
              }
              size="sm"
              className="footer_language"
            >
              ...
              {vaccineMinPeriodType && (
                <PeriodOptions
                  isPeriod={true}
                  periodType={vaccineMinPeriodType}
                />
              )}
            </Form.Select>
          </Tooltip>
        ...
        <div>
          <Tooltip title={translations.vaccine_maximum_period_type}>
            <Form.Select
              onChange={e =>
                vaccinePeriodical === 1
                  ? setVaccineMaxPeriodType(e.target.value)
                  : setVaccineMaxPeriodType('')
              }
              value={
                vaccineMaxPeriodType !== ''
                  ? (vaccineMaxPeriodType as string)
                  : ''
              }
              size="sm"
              className="footer_language"
        ...
        <div>
          <Tooltip title={translations.vaccine_maximum_recommend_date}>
            <Form.Select
              onChange={e =>
                vaccinePeriodical === 1
                  ? setVaccineMaxPeriod(parseInt(e.target.value, 10))
                  : setVaccineMaxPeriod(null)
              }
              value={
                vaccineMaxPeriod !== null ? vaccineMaxPeriod.toString() : ''
              }
              size="sm"
              className="footer_language"
            >
              {vaccinePeriodical !== 1 ? (
                ...
                <>
                  ...
                  {vaccineMaxPeriodType && (
                    <PeriodOptions
                      isPeriod={true}
                      periodType={vaccineMaxPeriodType}
                    />
                  )}
                </>
              )}
      ...
      <div className={`tempVaccineAddHeader`}>
        <span>{translations.national_code}</span>
        <span>{translations.vaccine_name}</span>
        <span>{translations.vaccine_is_periodical}</span>
        <span>{translations.vaccine_minimum_period_type}</span>
        <span>{translations.vaccine_minimum_recommend_date}</span>
        <span>{translations.vaccine_maximum_period_type}</span>
        <span>{translations.vaccine_maximum_recommend_date}</span>
        <span>{translations.vaccine_round}</span>
        <span>{translations.vaccine_description}</span>
      </div>
      <div className={`tempVaccineAddElem`}>
        <Tooltip
          title={`${vaccineNationalCode !== null ? vaccineNationalCode : ''}`}
        >
          <span>
        ...
        <Tooltip
          title={`${vaccinePeriodical !== 2 ? (vaccinePeriodical === 1 ? 'Periodic' : 'On Date') : ''}`}
        >
          <span>
        ...
        <Tooltip title={`${vaccineMinPeriodType}`}>
          <span>{`${vaccineMinPeriodType}`}</span>
        </Tooltip>
        <Tooltip
          title={`${vaccineMinPeriod !== null && vaccineMinPeriod !== 0 ? vaccineMinPeriod : ''}`}
        >
          <span>
        ...
        <Tooltip
          title={`${vaccinePeriodical === 1 ? vaccineMaxPeriodType : ''}`}
        >
          <span>
        ...
        <Tooltip
          title={`${vaccinePeriodical === 1 ? (vaccineMaxPeriod !== null && vaccineMaxPeriod !== 0 ? vaccineMaxPeriod : '') : ''}`}
        >
          <span>
        ...
        <Tooltip title={`${vaccineRound !== null ? vaccineRound : ''}`}>
          <span>{`${vaccineRound !== null ? vaccineRound : ''}`}</span>
        </Tooltip>

        <Tooltip title={`${vaccineDescription}`}>
          <span>{`${vaccineDescription}`}</span>
        </Tooltip>
      </div>
      <div>
        {vaccineNationalCode !== null &&
        ...
        vaccinePeriodical !== 2 &&
        vaccineMinPeriodType !== '' &&
        vaccineMinPeriod !== null &&
        vaccineMinPeriod !== 0 &&
        (vaccinePeriodical === 1
          ? vaccineMaxPeriodType !== '' &&
            vaccineMaxPeriod !== null &&
            vaccineMaxPeriod !== 0
          : true) &&
        vaccineRound !== null &&
        vaccineDescription !== '' ? (
...
export default CountryVaccineInformation;
