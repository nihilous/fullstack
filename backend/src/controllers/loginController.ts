...
const handleLogin = async (
  ...
  if (
    ...
    return res
      .status(400)
      .json({ message: 'Email and password are required', loginRes: 1 });
  }
  ...
  try {
    if (table === 'user') {
      ...
      if (users.length === 0) {
        return res
          .status(400)
          .json({ message: 'Invalid email or password', loginRes: 2 });
      }
      ...
      const tokenPayload = {
        userId: user.id,
        admin: isAdmin,
        userDetailIds: user.user_detail_ids
          ? user.user_detail_ids.split(',').map(Number)
          : [],
      };
      ...
      if (affectedUserRows === 1) {
        ...
        return res
          .status(400)
          .json({ message: 'Token save fail', loginRes: 3 });
      }
...
RowDataPacket & { user_detail_ids: string })[]
      ...
      if (affectedUserRows === 1) {
        res
          .status(201)
          .json({ message: 'JWT Expiration Refreshed', token: newToken });
      } else {
        return res
          .status(400)
          .json({ message: 'Token save fail', jwtRenewRes: 2 });
      }
