...
router.get(
  ...
  async (req: CustomRequest, res: Response) => {
    ...
    try {
      ...
      const processRows = (param_rows: Row[]): Row[] => param_rows.map((row) => {
        ...
        if (row.history_date === null) {
          if (row.vaccine_minimum_period_type === 'M') {
            ...
            newDate.setMonth(
              birthdate.getMonth() + row.vaccine_minimum_recommend_date,
            );
            row.expected_vaccine_minimum_recommend_date = formatDate(newDate);
          } else {
            const newDate = new Date(birthdate);
            newDate.setFullYear(
              birthdate.getFullYear() + row.vaccine_minimum_recommend_date,
            );
            row.expected_vaccine_minimum_recommend_date = formatDate(newDate);
          }
          ...
          if (row.vaccine_is_periodical) {
            if (row.vaccine_maximum_period_type === 'M') {
              ...
              newDate.setMonth(
                birthdate.getMonth()
                    + (row.vaccine_maximum_recommend_date || 0),
              );
