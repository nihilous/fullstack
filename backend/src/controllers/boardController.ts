...
router.get(
  ...
  async (req: CustomRequest, res: Response) => {
    ...
    if (isAttacked) {
      return res
        .status(400)
        .json({ message: 'Suspected to Attacking', boardGetRes: 2 });
    }
...
router.get(
  ...
  async (req: CustomRequest, res: Response) => {
    ...
    if (user_id !== token_id) {
      return res
        .status(403)
        .json({ message: 'No Authority', boardPostGetRes: 1 });
    }
    ...
    if (isAttacked) {
      ...
      addUpdateHostileList(clientIp as string, {
        id: user_id,
        token: token_id,
        post_id: injectionChecker(`${id}`),
      });

      return res
        .status(400)
        .json({ message: 'Suspected to Attacking', boardPostGetRes: 2 });
    }
      ...
      if (post[0].replies !== null) {
        for (let i = 0; i < post[0].replies.length; i++) {
          if (post[0].replies[i].reply_user_nickname !== null) {
            post[0].replies[i].reply_user_nickname = patternChecker(
              post[0].replies[i].reply_user_nickname,
            );
          }
          post[0].replies[i].reply_text = patternChecker(
            post[0].replies[i].reply_text,
          );
        }
      }
...
router.post('/', tokenExtractor, async (req: CustomRequest, res: Response) => {
  ...
  if (user_id !== token_id) {
    return res
      .status(403)
      .json({ message: 'No Authority', boardPostWriteRes: 1 });
  }
  ...
  if (
    checked_title === undefined
    || checked_title === ''
    || checked_text === undefined
    || checked_text === ''
  ) {
    return res
      .status(400)
      .json({ message: 'Title and Text are required', boardPostWriteRes: 2 });
  }
...
router.post(
  ...
  async (req: CustomRequest, res: Response) => {
    ...
    if (user_id !== token_id) {
      return res
        .status(403)
        .json({ message: 'No Authority', boardReplyWriteRes: 1 });
    }
    ...
    if (isAttacked) {
      return res
        .status(400)
        .json({ message: 'Suspected to Attacking', boardReplyWriteRes: 2 });
    }

    if (checked_text === undefined || checked_text === '') {
      return res
        .status(400)
        .json({ message: 'Text is required', boardReplyWriteRes: 3 });
    }
...
router.put(
  ...
  async (req: CustomRequest, res: Response) => {
    ...
    if (user_id !== token_id) {
      return res
        .status(403)
        .json({ message: 'No Authority', boardUpdateRes: 1 });
    }
    ...
    if (isAttacked) {
      return res
        .status(400)
        .json({ message: 'Suspected to Attacking', boardUpdateRes: 2 });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: 'Title and Text are required', boardUpdateRes: 3 });
    }
    ...
    try {
      ...
      if (affectedBoardRows === 1) {
        ...
        return res
          .status(400)
          .json({ message: 'No valid fields to update', boardUpdateRes: 4 });
      }
...
router.put(
  ...
  async (req: CustomRequest, res: Response) => {
    ...
    if (checked_text === undefined || checked_text === '') {
      return res
        .status(400)
        .json({ message: 'Text is required', boardUpdateRes: 3 });
    }
